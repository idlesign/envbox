...
from .detectors import DETECTORS, Detector, get_detector
from .envs import DEVELOPMENT, Environment, get_type


def get_environment(
        *,
        default: str | Environment | None = DEVELOPMENT,
        detectors: list[Detector] | None = None,
        detectors_opts: dict | None = None,
...
def import_by_environment(
    ...
    * `import_by_environment()` call in `settings.py` makes symbols from `settings_development.py`
    ...
    filedir = Path(settings_module.f_code.co_filename).parent

    if not package_name:
        package_name = filedir.name

        if package_name not in sys.modules.keys():
            # Last try to deduce maybe 'someproject.settings'.
            package_name = f'{filedir.parent.name}.{package_name}'
